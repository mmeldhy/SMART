                ...
                <div class="p-6">
                    ...
                    <form action="/admin/announcement/add" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="content" name="content" rows="6" class="shadow-sm focus:ring-green-500 focus:border-green-500 block w-full sm:text-sm border-gray-300 rounded-md wysiwyg-editor" data-name="content" required><?php echo isset($old['content']) ? htmlspecialchars($old['content']) : ''; ?></textarea>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="type" name="type" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm rounded-md">
                                <option value="general" <?php echo (isset($old['type']) && $old['type'] === 'general') ? 'selected' : ''; ?>>General</option>
                                <option value="important" <?php echo (isset($old['type']) && $old['type'] === 'important') ? 'selected' : ''; ?>>Important</option>
                                <option value="event" <?php echo (isset($old['type']) && $old['type'] === 'event') ? 'selected' : ''; ?>>Event</option>
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Upload an image related to the announcement (max 2MB, JPG/PNG only)</p>
                        </div>
                        
                        <div class="mb-4">
                            <label for="start_date" class="block text-sm font-medium text-gray-700 mb-1">Start Date</label>
                            <input type="date" id="start_date" name="start_date" value="<?php echo isset($old['start_date']) ? htmlspecialchars($old['start_date']) : date('Y-m-d'); ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm">
                        </div>
                        
                        <div class="mb-4">
                            <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date (Optional)</label>
                            <input type="date" id="end_date" name="end_date" value="<?php echo isset($old['end_date']) ? htmlspecialchars($old['end_date']) : ''; ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            <p class="mt-1 text-sm text-gray-500">Leave empty for announcements without an end date</p>
                        </div>
                        
                        <div class="flex items-center mb-4">
                            <input type="checkbox" id="is_pinned" name="is_pinned" value="1" <?php echo (isset($old['is_pinned']) && $old['is_pinned']) ? 'checked' : ''; ?> class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded">
